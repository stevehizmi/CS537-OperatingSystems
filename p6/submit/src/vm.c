...
// insert dont call decrypt
// end of decrypt  call insert
// Insert a page (should be guaranteed not already in queue)
// into the clock queue.
static void
clock_insert(int vpn, pte_t *pte)
{
struct proc *cur_proc = myproc();
    for (;;) {
        // First advance the hand.
        cur_proc->clock_hand = (cur_proc->clock_hand + 1) % CLOCKSIZE;

        // Found an empty slot.
        if (cur_proc->clock_q[cur_proc->clock_hand].vpn == -1) {
            cur_proc->clock_q[cur_proc->clock_hand].vpn = vpn;
            cur_proc->clock_q[cur_proc->clock_hand].pte = pte;
            break;

        // Else if the page in this slot does not have its ref
        // bit set, evict this one.
        } else if (!(*(cur_proc->clock_q[cur_proc->clock_hand].pte) & PTE_A)) {
            // Encrypt the evicted page.
	    //cprintf("---Evicting page VPN: %x\n",cur_proc->clock_q[cur_proc->clock_hand].vpn);
            mencrypt((char*)cur_proc->clock_q[cur_proc->clock_hand].vpn, 1);//(pte_t)cur_proc->clock_q[cur_proc->clock_hand].pte);
            // Put in the new page.
            cur_proc->clock_q[cur_proc->clock_hand].vpn = vpn;
            cur_proc->clock_q[cur_proc->clock_hand].pte = pte;
            break;
        ...
        // Else, clear the ref bit of the page in slot.
            ...
            *(cur_proc->clock_q[cur_proc->clock_hand].pte) &= (~PTE_A);
        }
    }
...
static void
clock_remove(int vpn)
{
    struct proc *cur_proc = myproc();
    int prev_tail = cur_proc->clock_hand;
    int match_idx = -1;

    // Search for the matching element.
    for (int i = 0; i < CLOCKSIZE; ++i) {
        int idx = (cur_proc->clock_hand + i) % CLOCKSIZE;
        if (cur_proc->clock_q[idx].vpn == vpn) {
            match_idx = idx;
            break;
        }
    }
    ...
    for (int idx = match_idx;
        ...
        cur_proc->clock_q[idx].vpn = cur_proc->clock_q[next_idx].vpn;
        cur_proc->clock_q[idx].pte = cur_proc->clock_q[next_idx].pte;
    }

    // Clear the element at prev_tail. Set clk_hand to
    // one entry to the left.
    cur_proc->clock_q[prev_tail].vpn = -1;
    cur_proc->clock_hand = cur_proc->clock_hand == 0 ? CLOCKSIZE - 1
                             : cur_proc->clock_hand - 1;
}
...
printbuff() {  
  ...
  for (int i=0; i<CLOCKSIZE;i+=1) {
    if (p->clock_q[i].vpn == -1) {
      cprintf("-1\n");
    } else {
      cprintf("%x\n",(p->clock_q[i].vpn));
    }
...
copyuvm(pde_t *pgdir, uint sz)
{
  ...
  for(i = 0; i < sz; i += PGSIZE){
    ...
    pte_t *kid = walkpgdir(d,(void*)i,0);
    if((*pte & (PTE_P))==0){
      *kid = *kid & ~PTE_P;
    } 
  }
...
int mdecrypt(char *virtual_addr) {
  ...
  if (virtual_addr >= (char*) KERNBASE) {
    //cprintf("mdecrypt failed, bad address\n");
    return -1;
  }
  //set the present bit to true and encrypt bit to false
  pte_t * pte = walkpgdir(mypd, virtual_addr, 0);
  if (!pte || (*pte == 0)) {
    //cprintf("mdecrypt failed, could not find pte.\n");
    return -1;
  }

  if ((*pte & PTE_E) == 0) {
    //cprintf("Trying to decrypt decrypted page, exiting\n");
    return -1;
  }
  ...
  *pte = *pte | PTE_P;
  ...
  //cprintf("Inserting decrpyted page: %x. Pg num: %x. New buffer:\n",virtual_addr,PPN(*pte));

  char * slider = virtual_addr;
  for (int offset = 0; offset < PGSIZE; offset++) {
    //if ((uint)virtual_addr >= 0x6000 && offset%0x100 == 0) {
//	  cprintf("%x\n",slider);
  //  }
    *slider = ~*slider;
    slider++;
  }
  clock_insert((int)virtual_addr, pte);
  //printbuff();
...
int mencrypt(char *virtual_addr, int len) {
  ...
  slider = virtual_addr;
  //cprintf("%x and next %d pages. Ppage:",slider, len);
  for (int i = 0; i < len; i++) {
    char * kvp = uva2ka(mypd, slider);
    if (!kvp) {
      slider += PGSIZE;
      //cprintf("Could not map page, skipping\n");
      continue;
    }
    ...
    if (*mypte & PTE_E) {//already encrypted
      slider += PGSIZE;
      //cprintf("This page is already encrypted, skipping\n");
      continue;
    }
    for (int offset = 0; offset < PGSIZE; offset++) {
...
int getpgtable(struct pt_entry* entries, int num, int wsetOnly) {
  ...
  for (void * i = (void*) PGROUNDDOWN(((int)me->sz)); i >= 0 && index < num; i-=PGSIZE) {
    ...
    for(int j = 0; j < CLOCKSIZE; j++){
      if(me->clock_q[j].vpn == (int)i){
       present = 1;
       break;
      }
    }
    if((wsetOnly == 1) && (present == 0)){
      continue;
    }
    if(((wsetOnly == 1) && (present == 1)) || (wsetOnly == 0)){
    ...
    if (curr_pte && *curr_pte) {//this page is allocated
      ...
      //cprintf("ppage %x has VA %x\n",entries[index].ppage,i);
      //cprintf("Access bit is curretly %x\n",(*curr_pte & PTE_A));
      index++;
    }
...
int dump_rawphymem(uint physical_addr, char * buffer) {
  ...
  uchar* curr_addr;
  curr_addr = P2V(physical_addr);
  int retval = copyout(myproc()->pgdir, (uint)buffer, (void*)curr_addr, PGSIZE);
  if (retval)
    return -1;
  //cprintf("First value of buffer while in dump method: %x\n",buffer[0]);
  //cprintf("Buffer locations: %x %x\n",buffer,PPN(buffer));
  return 0;
}
